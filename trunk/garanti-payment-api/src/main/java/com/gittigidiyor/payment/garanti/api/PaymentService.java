...
class PaymentService {
	...
	public static GarantiApiThreeDRequestInfo getThreeDRequestInfo(GarantiApiRequest garantiApiRequest) throws GarantiApiException{
		...
		try {
			...
			String secureHash = GarantiApiUtil.calculateHashAsDefaultAndUpperCase(
					garantiApiRequest.getProvisionPassword() + 
					GarantiApiUtil.getArrangedTerminalId(garantiApiRequest.getTerminalId()));
			
			String secure3dhash = GarantiApiUtil.calculateHashAsDefaultAndUpperCase(
					garantiApiRequest.getTerminalId() + 
					garantiApiRequest.getOrderId() + 
					amountStr + 
					garantiApiRequest.getSuccessUrl() + 
					garantiApiRequest.getFailureUrl() + 
					garantiApiRequest.getType() + 
					installmentStr + 
					garantiApiRequest.getStoreKey() + 
					secureHash);
			
			garantiApiThreeDRequestInfo = new GarantiApiThreeDRequestInfo(
					secure3dhash, GarantiApiConstants.API_VERSION_DEFAULT, amountStr, installmentStr);
